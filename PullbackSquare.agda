...
module PullbackSquare where
  ...
  substitute-2-cell :
    ...
    → is-a-pullback-square f g z₁ z₂ γ
    → is-a-pullback-square f g z₁ z₂ γ'
  substitute-2-cell {z₁ = z₁} {z₂ = z₂} H (the-induced-map-is-an-equivalence-by proof) =
    the-induced-map-is-an-equivalence-by
      (the-map _ is-an-equivalence-since-it-is-homotopic-to _
        by (λ x → (λ p → z₁ x and z₂ x are-in-the-same-fiber-by p) ⁎ H x)
        which-is-an-equivalence-by proof)
